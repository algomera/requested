			...
			<div class="inline-block min-w-full py-2 align-middle">
				<table class="min-w-full divide-y divide-gray-300">
					...
					@forelse($suppliers as $supplier)
						<tr class="hover:bg-gray-50" wire:key="{{ $supplier->id }}">
							...
							<td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 lg:pr-8">
								<div class="inline-flex items-center justify-end space-x-3">
									<button wire:click="$emit('openModal', 'pages.suppliers.edit', {{ json_encode(['supplier' => $supplier->id]) }})" type="button"
									...
									<button wire:click="$emit('openModal', 'pages.suppliers.show', {{ json_encode(['supplier' => $supplier->id]) }})"
									        type="button"
									        class="flex h-6 w-6 items-center justify-center rounded-md transition hover:bg-zinc-900/5">
										<x-heroicon-o-eye class="w-4 stroke-zinc-900"/>
									</button>
									@if($deletingId != $supplier->id)
										<button wire:key="deleting-{{ $supplier->id }}" wire:click="$set('deletingId', '{{ $supplier->id }}')" type="button"
										        class="flex h-6 w-6 items-center justify-center rounded-md transition hover:bg-zinc-900/5">
											<x-heroicon-o-trash class="w-4 stroke-zinc-900"/>
										</button>
									@else
										<button wire:key="confirm-{{ $supplier->id }}"
										        x-init="setTimeout(() => $wire.deletingId = null, 5000)"
										        wire:click="delete({{ $supplier->id }})" type="button"
										        class="flex h-6 w-6 items-center justify-center rounded-md transition hover:bg-zinc-900/5">
