	...
	class EditProduct extends ModalComponent
	{
		...
		public function save() {
			...
			$this->location->logs()->create([
				'user_id' => auth()->id(),
				'message' => "ha modificato la quantità di '{$this->product->name}' da {$this->old_quantity} a {$this->quantity} " . config('requested.products.units.' . $this->product->units) . " nell'ubicazione {$this->location->code}"
			]);
